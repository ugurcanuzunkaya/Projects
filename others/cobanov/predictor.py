...
class DepthEstimationModel:
    ...
    def save_colored_depth(self, depth_numpy, output_path):
        ...
        print(f"Colored depth map saved at {output_path}")

    def calculate_depthmap(self, image_path, output_path):
        input_image = Image.open(image_path).convert("RGB")
        print(f"Image {image_path} loaded successfully")

        depth_numpy = self.model.infer_pil(input_image)
        self.save_colored_depth(depth_numpy, output_path)

        return f"Depth map saved at {output_path}"


model = DepthEstimationModel()
model.calculate_depthmap("test_image.png", "Others/Cobanov/depth_map.png")
